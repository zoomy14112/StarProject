...
5 月 10 日，尝试复现 5 月 6 日的结果，失败。所幸当时我对那次训练得到权重进行备份，现在仍能加载那份权重进行推理，跑出相同指标；或许是因为学习率过大等原因，即使用相同的代码、相同的参数也无法训练得出相同指标的权重，每次训练得到的指标都比那一次低 3 ~ 6 个百分点。查询 bash 记录发现，此前那次得到非常优秀的权重的训练犯了个错误：未清除上次训练得到的权重。我认为的 129 轮训练，实则是 20 轮 + 129 轮，且前 20 轮的动量等优化器相关变量未能集成到后续训练中。这算什么……
